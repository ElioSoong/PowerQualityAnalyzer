...
bool IParser::addObjectRef(const ObjectRef& objectRef) const
{
    if (!m_loader) {
        return false;
    }

    return m_loader->addObjectRef(objectRef);
}
...
QVariant IParser::loadJsonFile(const QString& jsonFile, ObjectContext& parentContext, const QString& parentKey) const
{
    ...
    QList<ObjectContext*> dummy;

    // 此处在这之前有个大bug：
    // load(jsonFile, parentContext, parentKey, dummy, dummy)并未完整地执行解析，
    // 只是创建了对象，属性并未初始化！
    // 而load(jsonFile)这个版本则是完整的对外提供的接口。
    // [5/26/2017 CHENHONGHAO]
    return m_loader->load(jsonFile);
    //return m_loader->load(jsonFile, parentContext, parentKey, dummy, dummy);
}
...
{
    QString valueStringTemp = valueString;
    QString jsonKey;
    bool need = false;
    if (valueStringTemp.contains(":"))
    {
        need = true;
        QStringList list = valueStringTemp.split(":");
        jsonKey = list.at(0);
        valueStringTemp = list.at(1);
    }

    QStringList contents = valueStringTemp.split(QChar('.'), QString::KeepEmptyParts);
    if (need)
    {
        contents[0] = jsonKey + ":" + contents[0];
    }
    ...
    if (qObject == NULL)
    {
        ...
        // 对象名可能以下划线开头，类名只支持以大写字母开头 [5/25/2017 CHENHONGHAO]
        else if (!objectName.at(0).isUpper())
        {
            Object* object = NULL;
            if (objectName == QLatin1String("this"))
            {
                object = objectContext->parent();
                //qDebug() << "This is " << object->qObject()->objectName();
            }
            else if (objectName == QLatin1String("parent"))
            {
                object = objectContext->parent()->parent();
            }
            else
            {
                object = objectContext->findUpwards(objectName);
            }
...
bool ObjectCreator::parseIdKey(ObjectContext* objectContext, QObject* qObject) const
{
    ...
    QStringRef qtIdKey = m_idKey.midRef(m_idKey.startsWith(QLatin1Char('.')) ? 1 : 0);
    if (metaObject && (idPropertyIndex = metaObject->indexOfProperty(qtIdKey.toLatin1().constData())) >= 0)
    {
...
#if 0
        emit error(
            JsonLoader::ObjectCreatorError, 
            "Failed to create object with .type=" + metaTypeName
            );
#endif
    }
#if 1
...
bool RefKeyObjectCreator::parse(ObjectContext* objectContext) const
{
    ...
    objectContext->removeChild(keyIter);
    if (idValue.type() == QJsonValue::String &&
        (objectName.endsWith(QLatin1String(".json")) || objectName.startsWith(QLatin1String("`json`"))))
    {
        if (objectName.startsWith(QLatin1String("`json`")))
        {
            objectName.remove("`json`");
        }
        QString nestedJsonFilePath = objectName;
        ObjectContext* parent = objectContext->parent();
        ObjectContext parentContext = parent ? *parent : ObjectContext();
    ...
    else
    {
        ...
        // 对象上下文对应的QObject为空
        if (NULL == object->qObject())
        {
            error(
                JsonLoader::KeyParserError,
                QString("The QObject of referenced object is NULL: ") + objectName
                );
            return false;
        }
        objectContext->setQObject(object->qObject());

        sourceObject = object->qObject();
    ...
    bool ok = false;
    if (qObject)
        ok = parseIdKey(objectContext, qObject);

    ObjectRef objectRef(qObject, sourceObject, qObject->objectName(), objectName);
    ...
    while (iter != cend)
    {
        objectRef.overridedProperties.push_back(iter->first);
        ++iter;
    }
...
bool PropertyKeyParser::parse(ObjectContext* objectContext, KeyObjectContextMapConstIter keyIter) const
{
    ...
    if (firstValue.value().type() == QJsonValue::Array)
    {
        ...
        else if (qPropertyType == QMetaType::QVariantList || qPropertyType == QMetaType::QVariant)
        {
        ...
        for (; iter != cend; ++iter)
        {
            ...
            int valueVariantType = valueVariant.userType();
            if (valueVariant.canConvert<PropertyContext>())
            {
                PropertyContext targetPropertyContext = valueVariant.value<PropertyContext>();
                valueVariant = targetPropertyContext.read();
                if (valueVariant.userType() >= QVariant::UserType)
                {
                    // 此处主要应对自定义的对象广场无法正常适配QMetaType类型码的问题
                    if (valueVariant.canConvert(QMetaType::QObjectStar))
                    {
                        if (valueVariant.convert(QMetaType::QObjectStar))
    ...
    else
    {
        ...
        if (propertyVariantType != propertyMetaType)
        {
            if (propertyVariant.canConvert<PropertyContext>())
            {
                ...
                PropertyBinding* binding = targetPropertyContext.addObserver(qObject, qProperty);
...
#else
                    // 属性没有定义notify是很常见的情况，而且在addObserver时已经报错，此处不应该提示过高等级的错误 [6/29/2016 CHENHONGHAO]
#endif
                    //return false;
                }
    ...
    {
        // 详细诊断这个最常见的错误，输出有用的信息 [10/22/2016 CHH513]
        diagnosePropertyWriteError(objectContext, key, qProperty, propertyVariant);
        return false;
    }
...
void PropertyKeyParser::diagnosePropertyWriteError(
    ObjectContext* objectContext, 
    const QString& key, 
    QMetaProperty &qProperty,
    const QVariant &propertyVariant 
    ) const
{
    QString propertyVariantStr = propertyVariant.toString();
    if (propertyVariant.canConvert(QMetaType::QObjectStar))
    {
        QObject* propertyObject = propertyVariant.value<QObject*>();
        if (propertyObject)
        {
            QString objectName = propertyObject->objectName();
            if (!objectName.isEmpty())
                objectName.append(':');
            const QMetaObject* metaObject = propertyObject->metaObject();
            QString className = metaObject ? metaObject->className() : "<unknown>";
            propertyVariantStr = QString("%1%2@%3")
                .arg(objectName)
                .arg(className)
                .arg(qulonglong(propertyObject), sizeof(void*)*2, 16, QChar('0'));
        }
        else
            propertyVariantStr = "<null>";
    }
    if (propertyVariantStr.isEmpty())
        propertyVariantStr = "<?>";
    error(
        JsonLoader::KeyParserError, 
        QString("Failed to set Q_PROPRTY \"%1\" of %2 to %3.")
        .arg(key)
        .arg(objectContext->toString())
        .arg(propertyVariantStr)
        );
    error(
        JsonLoader::KeyParserError, 
        QString("\tProperyType=%1(%2), ValueType=%3(%4).")
        .arg(qProperty.typeName())
        .arg(qProperty.userType())
        .arg(propertyVariant.typeName())
        .arg(propertyVariant.userType())
        );
    if (qProperty.isValid() && qProperty.userType() == QMetaType::UnknownType)
    {
        const QMetaObject* metaObject = qProperty.enclosingMetaObject();
        QString className = metaObject ? metaObject->className() : "<unknown>";
        QString propertyType = qProperty.typeName();
        QString propertyClassName = propertyType;
        while (propertyClassName.endsWith('*'))
            propertyClassName.chop(1);
        int left = propertyClassName.indexOf('<');
        int right = propertyClassName.lastIndexOf('>');
        if (left >=0 && right >= 0)
            propertyClassName = propertyClassName.mid(left+1, right-left-1);
        while (propertyClassName.endsWith('*'))
            propertyClassName.chop(1);
        error(
            JsonLoader::KeyParserError, 
            QString(
            "\tYou MUST register meta-type:%1,"
            "and #include \"%2.h\" in \"%3.h\"."
            )
            .arg(propertyType)
            .arg(propertyClassName)
            .arg(className)
            );
    }
}
...
//             );
	...
	foreach(ObjectContext* value, values)
	{
		if (!value || value->value().type() == QJsonValue::Array)
			continue;

		QVariant targetMethodVariant = parseValue(*value, StringValueParser::MethodNameString);
...
int VariantArrayValueParser::parseArrayElementType(int propertyMetaTypeId, const QString& propertyMetaTypeName) const
{
    Q_ASSERT(0);
    return QMetaType::UnknownType;
}

QVariant VariantArrayValueParser::parseValue(const QVariantList& valueArray) const
{
    Q_ASSERT(0);
    return QVariant();
}
...
//             return false;
	...
	return allOk ? enumValue : QVariant();
}
...
QVariant PropertyNameStringValueParser::parse( ObjectContext* objectContext, const QString& valueString, const QStringList& tags ) const
{
    ...
    {
        // 枚举解析器在此之后，这里不能打印太多错误，否则会导致枚举误报错 [12/28/2016 CHENHONGHAO]
#if 0
        error(
            JsonLoader::StringValueParserError, 
    ...
    bool full = (!tags.isEmpty() && (tags.at(0) == "full-bind"));
    result.setFullBind(full);
...
QVariant MethodNameStringValueParser::parse( ObjectContext* objectContext, const QString& valueString, const QStringList& tags ) const
{
    ...
    {
        // 这里不能打印太多错误，否则可能会导致枚举、属性误报错 [12/28/2016 CHENHONGHAO]
...
{
//     if (valueString.endsWith("png", Qt::CaseInsensitive) ||
//         valueString.endsWith("jpg", Qt::CaseInsensitive) ||
//         valueString.endsWith("bmp", Qt::CaseInsensitive) ||
//         valueString.endsWith("ico", Qt::CaseInsensitive))
    {
        QPixmap pixmap;
        if (pixmap.load(valueString))
