...
** Latest modified Date:       2016/12/28
...
**                             V1.3.1: 完善属性绑定功能[2016/06/29]:
**                                     [1] 增加this关键字，用于对象自身属性的赋值或绑定
**                                     [2] 修正在使能了自定义ObjectFactory时，无法在数组类型的属性中使用this关键字的问题
**                                     [3] 降低属性绑定时未定义notify信号的警告等级
**                             V1.3.2: 增加从脚本直接载入QRegExp的pattern的能力[2016/08/06]
**                             V1.3.3: 支持属性类型是QVariant，而实际上通过object这个tag指定了一个对象这种场景[2016/09/23]
**                             V1.3.4: 初步支持与JsonScripter的协作，可以导出ObjectRef列表，但其他预期功能尚未完成[2016/10/21]
**                             V1.3.5: 支持以QVariant/QVariantList的形式定义Q_PROPERTY，并载入多维嵌套的纯数据数组[2016/10/21]
**                             V1.3.6: [1] 修正QRegExp类型的属性解析太靠后导致被EnumName解析器抢先错误解析的问题
**                                     [2] 不再使用QHash作为解析器的容器，从而保证解析器的强顺序
**                                     [3] 增加更方便的函数模板load
**
**--------------------------------------------------------------------------------------------------------
...
public: 
    ...
    QVariant load(
        ...
        int defaultMetaType = QMetaType::UnknownType
        );

    /*! 
     * 载入内存中的JSON数据（例如来自网络的、代码中的JSON），并直接转换为指定的数据类型
     * @param[in]  jsonData         内存中的JSON数据
     * @param[in]  parentKey        通常需要为该JSON数据指定一个Key，用于标识对象树的主分支
     * @param[in]  defaultMetaType  如果JSON对象未指定类型，则使用此默认类型创建对象（仅使用一次）
     * @return     加载得到的根对象或根数组
    ...
    {
        QVariant root = load(jsonData, parentKey, defaultMetaType);
        return root.value<T>();
    }
     ...
     */
    template <typename T>
    T load(const QString& jsonFile, int defaultMetaType = QMetaType::UnknownType)
    {
        QVariant root = load(jsonFile, defaultMetaType);
        return root.value<T>();
    }
     ...
     * @return     bool
     */
    bool remove(const QString& filePathOrParentKey, bool removeChildren = true);
     ...
     */
    bool addGlobalObject(QObject* object, bool allChildren);

    /*!
     * 判断对象是否已经是全局对象
     */
    bool existsInGlobal(const QObject* obj);
     ...
     * @return     操作成功返回true
     */
    bool addObjectRef(const ObjectRef& objectRef);
     ...
     * @return     操作成功返回true
     */
    bool removeObjectRef(const ObjectRef& objectRef);

    /** 
     * 获取当前已经添加的全部对象引用信息
     * @return     QList<ObjectRef> 当前已经添加的全部对象引用信息
     */
    QList<ObjectRef> objectRefs();
...
protected: 
     ...
     * @param[in]  parentKey          通常需要为该JSON数据指定一个Key，用于标识对象树的主分支
     * @param[in]  possibleObjectList 可能是对象的对象上下文（ObjectContext）列表，追加，原内容不清空
     * @param[in]  jsonObjectList     可能是JSON文件的对象上下文（ObjectContext）列表，追加，原内容不清空
     * @return     加载得到的根对象或根数组
     */
    QVariant load(    
        const QByteArray& jsonData, 
     ...
     * @param[in]  offset   需要转储的中心位置，
     *                      如果使用换行作为格式化方法，则转储前后各几行，否则转储前后各一段长度
     * @return     转储结果，可直接显示
     */
...
private:
    ...
    QList<ObjectRef>                m_objectRefs;             //!< 对象引用列表
    QList<PropertyBinding>          m_propertyBindings;       //!< 属性绑定列表
    QList<MethodConnection>         m_methodConnections;      //!< 方法绑定列表

    QList<ObjectCreator*>           m_objectCreators;         //!< 对象创建器容器
    QList<KeyParser*>               m_keyParsers;             //!< Key解析器容器
    QList<ArrayValueParser*>        m_arrayValueParsers;      //!< ArrayValue解析器容器
    QList<StringValueParser*>       m_stringValueParsers;     //!< StringValue解析器容器
    ...
    int                             m_defaultMetaType;        //!< 载入顶层JSON时，提供的默认MetaType提示
